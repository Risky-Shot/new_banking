...
RegisterCommand("banking", function()
	if nearBank and not banks[bankkey].closed and canUse then
		...
		SendNUIMessage({type = 'openGeneral', data = "bank"})
	...
	elseif nearATM and canUse then
		...
		ClearPedTasksImmediately(GLOBAL_PED)
		SetNuiFocus(true, true)
		SendNUIMessage({type = 'openGeneral', data = "atm"})
		TriggerServerEvent('bank:balance')
	end
end)
...
RegisterNetEvent('new_banking:client:SetBankState')
AddEventHandler('new_banking:client:SetBankState', function(BankId, bool)
...
AddEventHandler('currentbalance1', function(balance)
	local id = PlayerId()
	local playerName = ""
	ESX.TriggerServerCallback('new_banking:fetchData', function(data)
		if data then
